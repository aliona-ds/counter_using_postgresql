...
def counter_function():
    ...
    for i in range(10000):
        while (True):
            curr.execute("""SELECT counter, version FROM user_counter WHERE user_id = 1""")
            (counter, version) = curr.fetchone()
            counter = counter + 1
            curr.execute("""update user_counter set counter = %s, version = %s where user_id = %s and version = %s""",
